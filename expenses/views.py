...
def initialize_ml_model():
    global model, tfidf_vectorizer, stop_words
    if model is None:  # Only initialize if not already done
        data = pd.read_csv('dataset.csv')
        stop_words = set(stopwords.words('english'))
        
        # Preprocessing
        data['clean_description'] = data['description'].apply(lambda x: preprocess_text(x, stop_words))
...
def predict_category(description):
    global model, tfidf_vectorizer, stop_words
    initialize_ml_model()  # Make sure model is initialized
    
    # Preprocess the input description
    clean_description = preprocess_text(description, stop_words)
    
    # Transform the text using the fitted vectorizer
    X_new = tfidf_vectorizer.transform([clean_description])
