...
def income_summary(request):
    ...
    # Convert all times to date objects for consistent comparison
    today = timezone.now().date()
    one_week_ago = (today - timedelta(days=7))

    # Get all incomes for the user
    all_incomes = user.userincome_set.all()

    # Calculate daily income (for today)
    daily_income = sum(income.amount for income in all_incomes 
                      if income.date == today)

    # Calculate weekly income
    weekly_income = sum(income.amount for income in all_incomes 
                       if one_week_ago <= income.date <= today)

    # Calculate monthly income
    monthly_income = sum(income.amount for income in all_incomes 
                        if income.date.year == today.year 
                        and income.date.month == today.month)

    # Calculate yearly income
    yearly_income = sum(income.amount for income in all_incomes 
                       if income.date.year == today.year)
